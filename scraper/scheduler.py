...
Periodically scrapes and updates the menu database from @matrixfood Telegram channel.
"""
...
class MenuScheduler:
    ...
    def scrape_and_store(self):
        """Scrape the menu from Telegram channel and store in database."""
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Scraping @matrixfood channel...")

        try:
            menu_items = self.scraper.scrape_today_menu()
    ...
    def start(self):
        ...
        # Schedule periodic scraping every N hours
